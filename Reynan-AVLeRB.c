...
int REDBLACKfatorDeBalanceamentoo(struct rbNode *node)
{
  if (node)
  {
...
// FUNCAO PARA PEGAR A MÉDIA DE ROTACOES E EXIBIR O VALOR
int RBrot(int auxI, struct rbNode *raiz)
{
  ...
  printf("\tRED-BLACK -> QUANTIDADE ROTACAO DA REDBLACK: %d\n", auxI);
}

// FUNCAO PARA INSERIR UM NÓ NA VERMELHA E PRETA
int auxI = 0;
void insertion(int data)
{
  ...
  stack[ht] = root;
  dir[ht++] = 0;
  // LINK[0] Direita e LINK[1] Esquerda
  while (ptr != NULL)
  { // 20
    if (ptr->data == data)
    { // Se o valor que eu immplementar for igual
...
// FUNCAO PARA IMPRIMIR A MINHA REDBLACK TREE
...
// FUNÇÃO BUSCAR UM NÓ - DESNECESSÁRIO SEU USO NESTE TRABALHO
...
// FUNCAO PEGA FATOR DE BALANCEAMENTO
int aux5 = 0;
...
int preOrdem(No *raiz)
{
  ...
  else
  {
    ...
    // printf("\n\t\tQANTIDADE DE NO: %d\n", qtdNO);
    // printf("\n\n\t AVL -> %d° SOMA DOS FATORES DE CADA NÓ: %d\n", n1, aux7);
    // printf("\t %d° NIVEL DE BALANCEAMENTO NB: %.2f\n", n2, NB);
...
int REDBLACKprog_exp(struct rbNode *root)
{
  ...
  int left = REDBLACKalturaDaArv(root->link[0]);
  int right = REDBLACKalturaDaArv(root->link[1]);
  if (left > right)
  {
    fatb = left - right;
...
No *prog_exp(No *raiz, int dado)
{
  ...
  // root = raiz;
  REDBLACKprog_exp(root);
  // printf("\n\n\tMEDIA DE ROTAÇÕES FINAL: ");
  // insertion(dado);
}

// FUNCAO PEGA MINHA ENTRADA E APLICA OS SEUS VALORES
int dado;
